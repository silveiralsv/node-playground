function countingSort(arr: number[]) {
  const higherValue = Math.max(...arr);
  console.log("@@@@@  -> file: countingSort.ts:3 -> higherValue:", higherValue);
  const zeroArray = Array(arr.length).fill(0);
  for (let i = 0; i < zeroArray.length; i++) {
    const value = arr[i];
    zeroArray[value] = zeroArray[value] + 1;
  }
